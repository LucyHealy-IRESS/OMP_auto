...
Cypress.Commands.add("RunSearch", (SearchOptions, EntityData) => {
  ...
  var DefaultSearchCompletedCallback = function () { //This function exectutes only when the default search has finished loading
    ...
    //cy.wait(2000); //allow for the menu to fullopen so profile code can excecute
    ...
    cy.menuClick(SearchOptions.ThumbnailName, '#AmendClientSearch');
    cy.get(".AmendSearchPopup_Container").then(function () { //check amend search has opened before continuing
      cy.get(constantsSelectors.searchTextMatch).type(`{selectall}`).type(SearchParam);//clear added as in case of multi clients we were adding to the original search param
